                    ...
                    <div class="row">
                        ...
                        @foreach($languages as $key => $language)
                            <div id="lang-{{$key}}" class="col s12  mt-5">
                                ...
                                <div class="input-field ">
                                    {!! Form::text('description['.$key.']',$news->language($language->id) !== null ? $news->language($language->id)->description:  '',['class' => 'validate '. $errors->has('description.*') ? '' : 'valid']) !!}
...
{{--                                    {!! Form::label('content['.$key.']',__('admin.content')) !!}--}}
                                    {!! Form::textarea('content['.$key.']',$news->language($language->id) !== null ? $news->language($language->id)->content:  '',['class' => 'ckeditor validate '. ($errors->has('content.*') ? '' : 'valid')]) !!}
                                    @error('content.*')
                                    <small class="errorTxt4">
                                        <div class="error">
                                            {{$message}}
