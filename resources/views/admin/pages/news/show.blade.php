...
{{-- page title --}}
@section('title', $news->language()->title)
                ...
                <div class="display-flex media">
                    <div class="media-body">
                        <h6 class="media-heading">
                            <span class="users-view-name">{{$news->language()->title}} </span>
                        </h6>
                    </div>
                </div>
            </div>
    ...
    <div class="card">
        <div class="card-content">
            ...
            <div class="row mt-2">
                <div class="col s12">
                    <ul class="tabs">
                        @foreach($news->languages as $key => $language)
                            @if(isset($languages[$language->language_id]))
                                <li class="tab">
                                    <a href="#cat-{{$news->id}}-{{$language->language_id}}">
                ...
                <div class="col sm12 mt-2">
                    @foreach($news->languages as $key => $language)
                        @if(isset($languages[$language->language_id]))
                            <div id="cat-{{$news->id}}-{{$language->language_id}}"
                                 class="">
                                <table class="striped">
                                    <tbody>
                                    <tr>
